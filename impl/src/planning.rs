  ...
  fn produce_crate_contexts(
      ...
      for package in metadata.packages.iter() {
        ...
        let build_path = match settings.genmode {
          ...
          GenMode::Vendored => format!(
            "{}/vendor/{}-{}",
            settings.workspace_path, package.name, package.version
          ),
        };
        package_id_to_default_build_target.insert(
          package.id.clone(),
          format!("{}:{}", build_path, sanitized_name),
        );
    ...
    for node in sorted_nodes.into_iter() {
      ...
      crate_contexts.push(CrateContext {
        ...
        build_path: package_id_to_build_path
          .get(&own_package.id)
          .unwrap()
          .clone(),
